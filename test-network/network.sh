...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --platform linux/amd64 --rm hyperledger/fabric-tools:latest peer version | sed -ne 's/^ Version: //p')
...
function networkDown() {
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    ...
    DOCKER_SOCK2=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_ORG4_FILES} down --volumes --remove-orphans
